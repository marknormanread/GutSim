...
select  <- read.csv("phylogenetic-select.csv",header=1)
...
dfgenus <- findConvex(select$eaten.P, select$eaten.C, c("eaten.P","eaten.C"))
dfgenus$eaten.F <- median(select$eaten.F)

dfgenus2 <- findConvex(select$eaten.P, select$eaten.F, c("eaten.P","eaten.F"))
dfgenus2$eaten.C <- median(select$eaten.C)

dfgenus3 <- findConvex(select$eaten.C, select$eaten.F, c("eaten.C","eaten.F"))
dfgenus3$eaten.P <- median(select$eaten.P)
...
drawResponse <- function(name, training, newdata, graphName, summaryName, protFat=FALSE, protCarb=FALSE) 
{
	...
	pdf(graphName)
	par(cex=1.9)								# make text bigger on plot
	...
	xlab <- '' # turn off labels, temporarily. 
	ylab <- ''
	# image creates a pixelised plot. 
	image(x.new,y.new,surf,col=map[locs[1]:locs[2]],xlab=xlab,ylab=ylab, axes=FALSE)
	#image(x.new,y.new,surf,col=map[locs[1]:locs[2]],xlab=xlab,ylab=ylab, main=name,sub=sub,axes=FALSE)
	axis(1)								# draw the axes
	axis(2)
	contour(x.new,y.new,surf,add=TRUE,levels=pretty(range(mn,mx),8),labcex=1.9)		# labcex adjusts font size for contour lines. 
	dev.off()
...
######## SELECT (from the mouse-data-dup-removed.csv file) #########
# used in Cell Metabolism paper
for (j in 20:length(colnames(select)))
{
	colnames(select)[j]
	if (mean(unlist(select[j])) > 0.5)
	{
		name <- colnames(select)[j]
		graphLoc <- paste('select/', name, '.pdf', sep="")
		summaryLoc <- paste('select/', name, '.summary.txt', sep="")
		drawResponse(name,select,dfgenus,graphLoc,summaryLoc)

		name <- colnames(select)[j]
		graphLoc <- paste('select/', name, '-PF.pdf', sep="")
		drawResponse(name,select,dfgenus2,graphLoc,summaryLoc,protFat=TRUE)

		name <- colnames(select)[j]
		graphLoc <- paste('select/', name, '-CF.pdf', sep="")
		drawResponse(name,select,dfgenus3,graphLoc,summaryLoc,protCarb=TRUE)		
	}
}


# ######## GENUS #########
# for (j in 20:length(colnames(genus)))
# {
...
# ######### FAMILY #########
# for (j in 20:length(colnames(family)))
# {
...
# ######## CLASS #########
# for (j in 20:length(colnames(class)))
# {
