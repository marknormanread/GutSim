...
def AtestSlow(dist1, dist2):
    ...
    This is not an efficient implementation for distributions with large numbers of samples, but it's operation is
    intuitive.
    """
    equal = 0.0
    greater = 0.0
...
def ks_2samp(dist1, dist2):
    ...
    This implementation is not as fast as that of python's Scipy. However, I've had a great deal of trouble installing
    Scipy. I've tried to write this to be intuitively appealing.
    """
...
def gaussian(x, mu, sigma):
    """
    Find the value of a gaussian at point x, given values for mean (mu) and standard deviation (sigma).
    """
    k = 1.0 / (sigma * math.sqrt( 2 * math.pi ))    # coefficient to normalise Guassian (integral = 1)
    y = -1.0 * ((x - mu) * (x - mu)) / (2 * sigma * sigma)
    return k * math.exp(y)
...
def phi_gauss(x, mu, sigma):
    ...
    :return:
    """
    # These resources were of tremendous help:
    # http://www.johndcook.com/erf_and_normal_cdf.pdf
    # https://stackoverflow.com/questions/509994/best-way-to-write-a-python-function-that-integrates-a-gaussian/511155#511155
...
def testG():
    ...
    import numpy as np
    x = np.arange(-13.0, 13.0, 0.1)


    mu = 3.0
    sigma = 2.0
    integ = phi_gauss(1000, mu, sigma)
    print('integral = ' + str(integ))

    y = [gaussian(xi, mu, sigma) for xi in x]
    plt.clf()
    plt.plot(x, y)
    plt.show()
